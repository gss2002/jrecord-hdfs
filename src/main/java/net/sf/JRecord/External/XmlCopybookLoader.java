    ...
    */
    ...
    private void scanCopybook4Level(Element element) {
        ...
        if (lNodeList != null) {
            ...
            if (node.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
                Element childElement = (Element) node;
                String attrLevel = childElement.getAttribute(ATTR_LEVEL);
    ...
    private void insertElement(	Element element,
       ...
       if (element.hasAttribute(ATTR_NAME))  {
           ...
           if (element.hasAttribute(ATTR_LEVEL)) {
        	   level88 = element.getAttribute(ATTR_LEVEL).equals("88");
           }
           ...
           switch (splitCopybook) {
          	  case SPLIT_REDEFINE:
           	  ...
           	  break;
          	  case SPLIT_HIGHEST_REPEATING:
          		   if (Conversion.numTrim(getStringAttribute(element, ATTR_LEVEL)).equals(splitAtLevel)) {
          			  positionAdjustment = getIntAttribute(element, ATTR_POSITION) - 1;
           ...
           if (! print && level > 0 && (! level88) &&  level - 1 < groupName.size()) {
...
//            }
        ...
        ExternalField externalField = new ExternalField(
        	        getIntAttribute(element, ATTR_POSITION) + base - positionAdjustment,
